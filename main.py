...
class OnlineShopBot:
    ...
    async def init_database(self):
        ...
        await cursor.execute('''CREATE TABLE IF NOT EXISTS cart
                                ...
                                quantity INTEGER DEFAULT 1, selected_options TEXT, FOREIGN KEY (product_id) REFERENCES products(id))''')
        await cursor.execute('''CREATE TABLE IF NOT EXISTS orders
                                (id INTEGER PRIMARY KEY AUTOINCREMENT, user_id INTEGER NOT NULL, user_name TEXT, full_name TEXT, 
                                products TEXT NOT NULL, total_amount REAL NOT NULL, phone TEXT, address TEXT, payment_method TEXT, 
                                email TEXT, status TEXT DEFAULT 'pending', created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP)''')
        await cursor.execute('''CREATE TABLE IF NOT EXISTS users
                                (user_id INTEGER PRIMARY KEY, phone TEXT, address TEXT, email TEXT, full_name TEXT, blocked INTEGER DEFAULT 0)''')
        await cursor.execute('''CREATE TABLE IF NOT EXISTS promocodes
                                (id INTEGER PRIMARY KEY AUTOINCREMENT,code TEXT UNIQUE,discount INTEGER,max_uses INTEGER DEFAULT 100,
    ...
    async def load_states_from_db(self):
        async with self.conn.execute("SELECT user_id, state_data, promo_data FROM bot_states") as cursor:
            async for uid, s_data, p_data in cursor:
                if s_data:
                    try: self.user_states[uid] = json.loads(s_data)
                    except: pass
                if p_data:
                    try: self.user_promos[uid] = json.loads(p_data)
                    except: pass
    ...
    async def auto_save_state(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        async with self.conn.execute("SELECT user_id FROM bot_states WHERE user_id = ?", (user_id,)) as cursor:
            ...
            if exists:
                if s_data is None and p_data is None: await self.conn.execute("DELETE FROM bot_states WHERE user_id = ?", (user_id,))
                else: await self.conn.execute("UPDATE bot_states SET state_data = ?, promo_data = ? WHERE user_id = ?", (s_data, p_data, user_id))
            else:
                if s_data or p_data: await self.conn.execute("INSERT INTO bot_states (user_id, state_data, promo_data) VALUES (?, ?, ?)", (user_id, s_data, p_data))
        await self.conn.commit()
    ...
    def generate_receipt(self, order_id, user_name, email, phone, address, payment, products_list, total, date, receipt_format='html'):
        shipping_label = self.get_text('shipping_label_ukraine') if SHIPPING_MODE == 'UKRAINE' else self.get_text('shipping_label_international')
        ...
        for item in products_list:
            opts_str = f" ({', '.join([str(v) for v in item.get('selected_options', {}).values()])})" if item.get('selected_options') else ""
            item_total = item.get('total', 0)
            calc_subtotal += item_total
            products_text += product_line_format.format(emoji=item.get('emoji', '📦'), name=escaper(item.get('name', self.get_text('unknown'))), opts=escaper(opts_str), quantity=item.get('quantity', 1), total=item_total, symbol=CURRENCY_SYMBOL)
        ...
        return self.get_text('receipt', bold_start=bold_start, bold_end=bold_end, order_id=order_id, user_name=escaper(user_name), email=escaper(email), phone=escaper(str(phone)), shipping_label=shipping_label, address=escaper(address), payment=payment, products_text=products_text, total=total, date=date, symbol=CURRENCY_SYMBOL)
    ...
    def get_variant_type_keyboard(self):
        return InlineKeyboardMarkup([
            [InlineKeyboardButton(self.get_text('size_variant'), callback_data="vartype_Size"), InlineKeyboardButton(self.get_text('color_variant'), callback_data="vartype_Color")],
            [InlineKeyboardButton(self.get_text('memory_variant'), callback_data="vartype_Memory"), InlineKeyboardButton(self.get_text('volume_variant'), callback_data="vartype_Volume")],
            [InlineKeyboardButton(self.get_text('weight_variant'), callback_data="vartype_Weight"), InlineKeyboardButton(self.get_text('shoe_size_variant'), callback_data="vartype_ShoeSize")],
    ...
    def format_date(self, date_input):
        ...
        except Exception: return str(date_input)[:16]
    ...
    async def check_user_blocked(self, update: Update, context: ContextTypes.DEFAULT_TYPE) -> bool:
        if await self.is_user_blocked(update.effective_user.id):
            if update.callback_query: await update.callback_query.answer(self.get_text('user_blocked_inline'), show_alert=True)
            elif update.message: await update.message.reply_text(self.get_text('user_blocked'))
            return True
        return False
    ...
    async def restore_stock(self, order_id):
        ...
        if result and result[0]:
            try: products = json.loads(result[0])
            except: return
            ...
            for item in products:
                ...
                if not product_id and 'name' in item:
                    async with self.conn.execute("SELECT id FROM products WHERE name = ?", (item['name'],)) as cursor:
                        res = await cursor.fetchone()
                        if res: product_id = res[0]

                if product_id and quantity:
                    await self.conn.execute("UPDATE products SET stock = stock + ? WHERE id = ?", (quantity, product_id))
                    if sel_opts:
                        async with self.conn.execute("SELECT variants FROM products WHERE id = ?", (product_id,)) as cursor:
                        ...
                        if row and row[0]:
                            try:
                                ...
                                if changed:
                                    ...
                                    await self.conn.execute("UPDATE products SET variants = ? WHERE id = ?", (new_json, product_id))
                            except Exception as e:
    ...
    async def user_cancel_order(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        async with self.conn.execute("SELECT status FROM orders WHERE id = ? AND user_id = ?", (order_id, uid)) as cursor:
            row = await cursor.fetchone()

        if not row: return await query.answer(self.get_text('invalid_request'))
        if row[0] in ('cancelled', 'delivered'): return await query.answer(self.get_text('order_already_delivered_or_canceled'))

        await self.restore_stock(order_id)
        await self.conn.execute("UPDATE orders SET status = 'cancelled' WHERE id = ?", (order_id,))
        await self.conn.commit()

        for admin_id in ADMIN_IDS:
            try: await context.bot.send_message(chat_id=admin_id, text=self.get_text('customer_canceled_order', order_id=order_id), parse_mode=ParseMode.MARKDOWN)
    ...
    async def build_main_keyboard(self, user_id):
        ...
        if int(user_id) in ADMIN_IDS:
            return InlineKeyboardMarkup([
                ...
                [InlineKeyboardButton(cart_text, callback_data="cart"), InlineKeyboardButton(self.get_text('my_profile_button'), callback_data="my_profile")]
            ])
    ...
    async def show_main_menu(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        if int(user_id) in ADMIN_IDS:
            ...
            missing = await self.get_profile_completion_status(user_id)
            if missing:
                ...
                if "address" in missing: labels.append(self.get_text('missing_address_ukraine' if SHIPPING_MODE == 'UKRAINE' else 'missing_address_international'))
                ...
                welcome_text = f"{parts[0]}\n\n{promo_block}\n\n{parts[1]}" if len(parts) > 1 else f"{promo_block}\n\n{base_welcome}"
            else:
                welcome_text = base_welcome

        reply_markup = await self.build_main_keyboard(user_id)

        if update.callback_query:
            try: await update.callback_query.edit_message_text(welcome_text, reply_markup=reply_markup, parse_mode=ParseMode.HTML)
            except Exception: pass
        else:
    ...
    async def show_help(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        if await self.check_user_blocked(update, context): return
        text = STORE_MESSAGES[SHIPPING_MODE]['help'].format(shop_name=SHOP_NAME, support=SUPPORT_USER, channel=CHANNEL_LINK)
    ...
    async def ask_search(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        msg = await query.edit_message_text(self.get_text('search_prompt'), reply_markup=InlineKeyboardMarkup(keyboard), parse_mode="HTML")
        self.user_states[user_id]['msg_id'] = msg.message_id

    async def perform_search(self, update: Update, context: ContextTypes.DEFAULT_TYPE, search_query: str, page: int = 1):
        user_id = update.effective_user.id
        chat_id = update.effective_chat.id

        search_pattern = f"%{search_query}%"
        async with self.conn.execute("SELECT COUNT(*) FROM products WHERE is_active = 1 AND name LIKE ?", (search_pattern,)) as cursor:
            row = await cursor.fetchone()
        ...
        if total_items == 0:
            ...
            if getattr(update, "callback_query", None):
                ...
                m = await context.bot.send_message(chat_id=chat_id, text=text, reply_markup=InlineKeyboardMarkup(keyboard), parse_mode="HTML")
            return

        total_pages = (total_items + ITEMS_PER_PAGE - 1) // ITEMS_PER_PAGE
        offset = (page - 1) * ITEMS_PER_PAGE

        async with self.conn.execute("SELECT id, name, price, emoji, variants FROM products WHERE is_active = 1 AND name LIKE ? LIMIT ? OFFSET ?", (search_pattern, ITEMS_PER_PAGE, offset)) as cursor:
        ...
        for p_id, name, base_price, emoji, variants_json in products_data:
            ...
            if variants_json:
                try:
                    ...
                    pass
            keyboard.append([InlineKeyboardButton(f"{emoji or '📦'} {name} - {display_price}", callback_data=f"product_{p_id}_{page}_1")])

        nav = []
        if page > 1: nav.append(InlineKeyboardButton(self.get_text('prev_button'), callback_data=f"search_page_{page - 1}"))
        if page < total_pages: nav.append(InlineKeyboardButton(self.get_text('next_button'), callback_data=f"search_page_{page + 1}"))
        if nav: keyboard.append(nav)

        keyboard.append([InlineKeyboardButton(self.get_text('back_to_catalog_button_2'), callback_data="catalog")])

        if getattr(update, "callback_query", None):
            try: await update.callback_query.edit_message_text(text=text, reply_markup=InlineKeyboardMarkup(keyboard), parse_mode="HTML")
            except: pass
        else:
            m = await context.bot.send_message(chat_id=chat_id, text=text, reply_markup=InlineKeyboardMarkup(keyboard), parse_mode="HTML")
    ...
    async def handle_search_pagination(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        if match:
            ...
            if query_str: await self.perform_search(update, context, query_str, int(match.group(1)))
            else: await self.show_main_menu(update, context)
    ...
    async def show_catalog(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        async with self.conn.execute("SELECT COUNT(DISTINCT category) FROM products WHERE is_active = 1") as cursor:
            row = await cursor.fetchone()
            total_items = row[0] if row else 0

        if total_items == 0:
            try: await query.edit_message_text(self.get_text('catalog_empty'), reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton(self.get_text('main_menu_button'), callback_data="main_menu")]]), parse_mode="HTML")
            except: pass
            return
        ...
        async with self.conn.execute("SELECT DISTINCT category FROM products WHERE is_active = 1 ORDER BY category ASC LIMIT ? OFFSET ?", (ITEMS_PER_PAGE, offset)) as cursor:
        ...
        if LICENSE_TYPE == "Pro": keyboard.append([InlineKeyboardButton(self.get_text('search_button'), callback_data="ask_search")])
    ...
    async def show_category_products(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        async with self.conn.execute("SELECT COUNT(*) FROM products WHERE category = ? AND is_active = 1", (category,)) as cursor:
            row = await cursor.fetchone()
            total_items = row[0] if row else 0
        ...
        async with self.conn.execute("SELECT id, name, price, emoji, variants FROM products WHERE category = ? AND is_active = 1 LIMIT ? OFFSET ?", (category, ITEMS_PER_PAGE, offset)) as cursor:
            products_data = await cursor.fetchall()
        ...
        for p_id, name, base_price, emoji, variants_json in products_data:
            ...
            if variants_json:
                try:
                    ...
                    pass
            keyboard.append([InlineKeyboardButton(f"{emoji or '📦'} {name} - {display_price}", callback_data=f"product_{p_id}_{prod_page}_{cat_page}")])

        nav = []
        if prod_page > 1: nav.append(InlineKeyboardButton(self.get_text('prev_button'), callback_data=f"category_{category}_{prod_page - 1}_{cat_page}"))
        if prod_page < total_pages: nav.append(InlineKeyboardButton(self.get_text('next_button'), callback_data=f"category_{category}_{prod_page + 1}_{cat_page}"))
        if nav: keyboard.append(nav)
    ...
    async def show_product(self, update: Update, context: ContextTypes.DEFAULT_TYPE, product_id_override=None):
        ...
        if product_id_override:
            ...
            try:
                if parts[0] in ['product', 'prod']:
                    product_id, prod_page, cat_page = int(parts[1] if parts[0] == 'product' else parts[2]), int(parts[2] if parts[0] == 'product' else parts[3]), int(parts[3] if parts[0] == 'product' else parts[4])
                else:
                    product_id, prod_page, cat_page = int(parts[1]), 1, 1
            except:
                product_id, prod_page, cat_page = int(parts[1]), 1, 1

        async with self.conn.execute("SELECT * FROM products WHERE id = ?", (product_id,)) as cursor:
        ...
        stock = product['stock']
        stock_status = self.get_text('in_stock') if stock > 5 else (self.get_text('low_stock', stock=stock) if stock > 0 else self.get_text('out_of_stock'))

        async with self.conn.execute("SELECT SUM(quantity) FROM cart WHERE user_id = ? AND product_id = ?", (user_id, product_id)) as cursor:
            res = await cursor.fetchone()
            in_cart = res[0] or 0 if res else 0

        variants_display = ""
        if product['variants']:
            try:
                for v_type, options in json.loads(product['variants']).items():
                    variants_display += self.get_text('variant_display', v_type=v_type, opt_list=', '.join(map(str, options.keys())))
            except: pass
        ...
        keyboard = [
            [InlineKeyboardButton(self.get_text('btn_minus'), callback_data=f"prod_minus_{product_id}_{prod_page}_{cat_page}"),
             InlineKeyboardButton(self.get_text('btn_plus'), callback_data=f"prod_plus_{product_id}_{prod_page}_{cat_page}")],
        ...
        try:
            ...
            if 'images' in product.keys() and product['images']:
                try: images = json.loads(product['images'])
                except: pass

            if not images and product['image_url']: images = [product['image_url']]
            ...
            if len(images) > 1:
                if is_update:
                    ...
                    if 'album_msgs' in state and state['album_msgs']:
                        try: await context.bot.edit_message_caption(chat_id=query.message.chat_id, message_id=state['album_msgs'][0], caption=text, parse_mode="HTML")
                        except: pass
                    try: await query.edit_message_reply_markup(reply_markup=InlineKeyboardMarkup(keyboard))
                    except: pass
                else:
                    try: await query.message.delete()
                    except: pass
                    media_group = [InputMediaPhoto(media=img, caption=text if i == 0 else "", parse_mode="HTML") for i, img in enumerate(images[:10])]
                    ...
                    await context.bot.send_message(chat_id=query.message.chat_id, text=self.get_text('choose_action'), reply_markup=InlineKeyboardMarkup(keyboard), parse_mode="HTML")

            elif len(images) == 1:
                await self.cleanup_albums(user_id, context, query.message.chat_id)
                if is_update:
                    try: await query.edit_message_caption(caption=text, reply_markup=InlineKeyboardMarkup(keyboard), parse_mode="HTML")
                    except: await query.edit_message_text(text=text, reply_markup=InlineKeyboardMarkup(keyboard), parse_mode="HTML")
                else:
                    try: await query.message.delete()
                    except: pass
                    await context.bot.send_photo(chat_id=query.message.chat_id, photo=images[0], caption=text, reply_markup=InlineKeyboardMarkup(keyboard), parse_mode="HTML")
            else:
                await self.cleanup_albums(user_id, context, query.message.chat_id)
                try: await query.edit_message_text(text=text, reply_markup=InlineKeyboardMarkup(keyboard), parse_mode="HTML")
                except: pass
    ...
    async def handle_product_action(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        try:
            ...
            if len(parts) >= 5:
                ...
                self.user_states[user_id]['prod_page'], self.user_states[user_id]['cat_page'] = int(parts[3]), int(parts[4])
        except: return await query.answer(self.get_text('error_parsing_data'))

        async with self.conn.execute("SELECT stock, variants FROM products WHERE id = ?", (product_id,)) as cursor:
            row = await cursor.fetchone()
        ...
        if action_type == "plus":
            ...
            async with self.conn.execute("SELECT quantity FROM cart WHERE user_id = ? AND product_id = ?", (user_id, product_id)) as cursor:
                res = await cursor.fetchone()
                current_qty = res[0] if res else 0

            if current_qty < stock:
                if current_qty == 0: await self.conn.execute("INSERT INTO cart (user_id, product_id, quantity) VALUES (?, ?, 1)", (user_id, product_id))
                else: await self.conn.execute("UPDATE cart SET quantity = quantity + 1 WHERE user_id = ? AND product_id = ?", (user_id, product_id))
            else:
                return await query.answer(self.get_text('stock_limit', limit=stock), show_alert=True)

        elif action_type == "minus":
            async with self.conn.execute("SELECT id, quantity FROM cart WHERE user_id = ? AND product_id = ? ORDER BY id DESC LIMIT 1", (user_id, product_id)) as cursor:
                target = await cursor.fetchone()
            if target:
                if target[1] > 1: await self.conn.execute("UPDATE cart SET quantity = quantity - 1 WHERE id = ?", (target[0],))
                else: await self.conn.execute("DELETE FROM cart WHERE id = ?", (target[0],))
            else: return await query.answer(self.get_text('cart_empty_2'))
    ...
    async def start_variant_selection(self, update: Update, context: ContextTypes.DEFAULT_TYPE, product_id):
        ...
        try: variants_data = json.loads(row['variants'])
        except: return await self.add_item_to_cart_db(update, context, product_id, None)

        priority_keys = [self.get_text(k) for k in ['color_variant_key', 'colour_variant_key', 'color_variant_key_uk', 'color_variant_key_ru', 'size_variant_key', 'size_variant_key_uk', 'size_variant_key_ru']]
        sorted_keys = sorted(variants_data.keys(), key=lambda k: priority_keys.index(k.lower()) if k.lower() in priority_keys else 999)

        self.user_states[user_id] = {'step': 'selecting_variant', 'product_id': product_id, 'variant_keys': sorted_keys, 'current_key_index': 0, 'variants_data': variants_data, 'selected_options': {}}
    ...
    async def ask_next_variant(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        if not state or 'variant_keys' not in state:
            try: await update.callback_query.answer(self.get_text('session_expired'))
            except: pass
            return
        ...
        if v_type_localized == f"_type_{current_key}_": v_type_localized = current_key

        text = self.get_text('select_variant', current_key=v_type_localized)
        query = update.callback_query

        try:
            if query.message.photo: await query.edit_message_caption(caption=text, reply_markup=InlineKeyboardMarkup(final_keyboard), parse_mode=ParseMode.HTML)
            else: await query.edit_message_text(text=text, reply_markup=InlineKeyboardMarkup(final_keyboard), parse_mode=ParseMode.HTML)
        except Exception:
            try: await query.message.delete()
            except: pass
            await context.bot.send_message(chat_id=update.effective_chat.id, text=text, reply_markup=InlineKeyboardMarkup(final_keyboard), parse_mode=ParseMode.HTML)
    ...
    async def handle_variant_selection_user(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        if query.data == "cancel_selection":
            ...
            if state and 'product_id' in state: await self.show_product(update, context, product_id_override=state['product_id'])
            else:
                try: await query.message.delete()
                except: pass
            return
    ...
    async def show_profile(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        if update.callback_query: await update.callback_query.edit_message_text(text, reply_markup=keyboard, parse_mode="HTML")
        else: await context.bot.send_message(chat_id=update.effective_chat.id, text=text, reply_markup=keyboard, parse_mode="HTML")
    ...
    async def handle_delete_profile_data(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        await self.conn.execute(f"UPDATE users SET {db_field} = NULL WHERE user_id = ?", (query.from_user.id,))
        await self.conn.commit()
    ...
    async def _edit_user_profile_attribute(self, update: Update, context: ContextTypes.DEFAULT_TYPE, field: str, prompt: str):
        query = update.callback_query
        await query.answer()
        self.user_states[update.effective_user.id] = {'step': f'waiting_{field}_profile', 'msg_id': query.message.message_id}
        await query.edit_message_text(prompt, reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton(self.get_text('cancel_button'), callback_data="my_profile")]]), parse_mode="HTML")
    ...
    async def edit_address(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        await self._edit_user_profile_attribute(update, context, "address", self.get_text('enter_address_ukraine' if SHIPPING_MODE == 'UKRAINE' else 'enter_address_international'))
    ...
    async def profile_delete_menu(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        if await self.check_user_blocked(update, context): return
        query = update.callback_query
        async with self.conn.execute("SELECT full_name, email, address, phone FROM users WHERE user_id = ?", (update.effective_user.id,)) as cursor:
            row = await cursor.fetchone()
        ...
        keyboard = []
        if row[0]: keyboard.append([InlineKeyboardButton(self.get_text('delete_name_btn'), callback_data="delete_profile_full_name")])
        if row[1]: keyboard.append([InlineKeyboardButton(self.get_text('delete_email_btn'), callback_data="delete_profile_email")])
        if row[2]: keyboard.append([InlineKeyboardButton(self.get_text('delete_shipping_ukr_btn' if SHIPPING_MODE == 'UKRAINE' else 'delete_shipping_int_btn'), callback_data="delete_profile_address")])
        if row[3]: keyboard.append([InlineKeyboardButton(self.get_text('delete_phone_btn'), callback_data="delete_profile_phone")])
        keyboard.append([InlineKeyboardButton(self.get_text('back_to_profile_btn'), callback_data="my_profile")])

        try: await query.message.delete()
        except: pass
        await context.bot.send_message(chat_id=update.effective_chat.id, text=self.get_text('profile_delete_title'), reply_markup=InlineKeyboardMarkup(keyboard), parse_mode="HTML")
    ...
    async def handle_profile_input(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        if state['step'] == 'waiting_full_name_profile':
            ...
            if len(text.split()) < min_words: return await self._send_error(chat_id, 'err_invalid_name', error_kb, state, context)
            await self.conn.execute("UPDATE users SET full_name = ? WHERE user_id = ?", (text, user_id))

        elif state['step'] == 'waiting_email_profile':
            if not re.fullmatch(r"[^@\s]+@[^@\s]+\.[^@\s]+", text): return await self._send_error(chat_id, 'err_invalid_email', error_kb, state, context)
            await self.conn.execute("UPDATE users SET email = ? WHERE user_id = ?", (text, user_id))

        elif state['step'] == 'waiting_phone_profile':
            if not (re.fullmatch(r"^\+380\d{9}$", text) if SHIPPING_MODE == 'UKRAINE' else re.fullmatch(r"^\+\d{10,15}$", text)):
                return await self._send_error(chat_id, 'err_invalid_phone', error_kb, state, context)
            await self.conn.execute("UPDATE users SET phone = ? WHERE user_id = ?", (text, user_id))
            ...
            if not is_valid: return await self._send_error(chat_id, err_key, error_kb, state, context)
            await self.conn.execute("UPDATE users SET address = ? WHERE user_id = ?", (text, user_id))
    ...
    async def _send_error(self, chat_id, error_key, kb, state, context):
        m = await context.bot.send_message(chat_id=chat_id, text=self.get_text(error_key), reply_markup=kb, parse_mode="HTML")
    ...
    async def show_cart(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        async with self.conn.execute('SELECT c.id, p.name, p.price, c.quantity, p.emoji, c.selected_options, p.variants, p.id FROM cart c JOIN products p ON c.product_id = p.id WHERE c.user_id = ?', (user_id,)) as cursor:
            cart_items = await cursor.fetchall()

        if not cart_items:
            text = STORE_MESSAGES[SHIPPING_MODE]['cart_empty']
            keyboard = [[InlineKeyboardButton(self.get_text('go_to_catalog_button'), callback_data="catalog")], [InlineKeyboardButton(self.get_text('my_orders_button_2'), callback_data="my_orders")], [InlineKeyboardButton(self.get_text('main_menu_button'), callback_data="main_menu")]]
            try:
                if query and getattr(query.message, 'photo', None):
                    await query.message.delete()
                    await context.bot.send_message(chat_id=chat_id, text=text, reply_markup=InlineKeyboardMarkup(keyboard), parse_mode=ParseMode.HTML)
                elif query: await query.edit_message_text(text=text, reply_markup=InlineKeyboardMarkup(keyboard), parse_mode=ParseMode.HTML)
                else: await context.bot.send_message(chat_id=chat_id, text=text, reply_markup=InlineKeyboardMarkup(keyboard), parse_mode=ParseMode.HTML)
            except Exception: await context.bot.send_message(chat_id=chat_id, text=text, reply_markup=InlineKeyboardMarkup(keyboard), parse_mode=ParseMode.HTML)
            return

        total_amount, text, keyboard = 0, "", []
        promo_msg = self.user_states.get(user_id, {}).pop('promo_msg', None)
        if promo_msg: text += promo_msg + "\n\n"
        ...
        for cart_id, name, base_price, quantity, emoji, opts_json, variants_json, product_id in cart_items:
            ...
            keyboard.append([InlineKeyboardButton(self.get_text('btn_minus'), callback_data=f"cart_minus_{cart_id}"), InlineKeyboardButton(f"{name} ({quantity})", callback_data=f"product_{product_id}"), InlineKeyboardButton(self.get_text('btn_plus'), callback_data=f"cart_plus_{cart_id}")])
        ...
        if active_promo:
            ...
            text += f"\n" + self.get_text('cart_discount_info', discount=active_promo['discount'], discount_amount=round(discount_amount, 2))
        ...
        keyboard.extend([
            ...
            [InlineKeyboardButton(self.get_text('clear_cart_button'), callback_data="clear_cart"), InlineKeyboardButton(self.get_text('back_to_catalog_button'), callback_data="catalog")],
            [InlineKeyboardButton(self.get_text('main_menu_button'), callback_data="main_menu")]
        ])
    ...
    async def handle_cart_update(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        async with self.conn.execute("SELECT c.quantity, c.selected_options, p.stock, p.variants FROM cart c JOIN products p ON c.product_id = p.id WHERE c.id = ?", (cart_id,)) as cursor:
            row = await cursor.fetchone()
        ...
        if "plus" in action:
            if current_qty < limit: await self.conn.execute("UPDATE cart SET quantity = ? WHERE id = ?", (current_qty + 1, cart_id))
            else: return await query.answer(self.get_text('stock_limit', limit=limit), show_alert=True)
        elif "minus" in action:
            if current_qty > 1: await self.conn.execute("UPDATE cart SET quantity = ? WHERE id = ?", (current_qty - 1, cart_id))
            else: await self.conn.execute("DELETE FROM cart WHERE id = ?", (cart_id,))
    ...
    async def add_item_to_cart_db(self, update: Update, context: ContextTypes.DEFAULT_TYPE, product_id, options):
        ...
        async with self.conn.execute("SELECT id, quantity FROM cart WHERE user_id = ? AND product_id = ? AND selected_options IS ?", (user_id, product_id, options_json)) as cursor:
            cart_row = await cursor.fetchone()
        current_in_cart = cart_row[1] if cart_row else 0

        if current_in_cart + 1 > limit:
            if update.callback_query: await update.callback_query.answer(self.get_text('limit_reached', limit=limit), show_alert=True)
            return await self.show_product(update, context, product_id_override=product_id)

        if cart_row:
            await self.conn.execute("UPDATE cart SET quantity = quantity + 1 WHERE id = ?", (cart_row[0],))
        else:
            await self.conn.execute("INSERT INTO cart (user_id, product_id, quantity, selected_options) VALUES (?, ?, 1, ?)", (user_id, product_id, options_json))
    ...
    async def clear_cart(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        async with self.conn.execute("SELECT COUNT(*) FROM cart WHERE user_id = ?", (user_id,)) as cursor:
            row = await cursor.fetchone()
            items_count = row[0] if row else 0
    ...
    async def checkout(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        async with self.conn.execute("SELECT product_id FROM cart WHERE user_id = ?", (user_id,)) as cursor:
            if not await cursor.fetchone(): return await query.answer(self.get_text('cart_empty_3'))
        ...
        if has_saved_data:
            ...
            await query.edit_message_text(text=self.get_text('checkout_step_1'), reply_markup=InlineKeyboardMarkup(keyboard), parse_mode="HTML")
            self.user_states[user_id]['msg_id'] = query.message.message_id
        else:
            self.user_states.setdefault(user_id, {})
            self.user_states[user_id].update({'full_name': None, 'email': None, 'address': None, 'phone': None, 'from_profile': False})
    ...
    async def use_profile_data(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        self.user_states.setdefault(user_id, {}).update({'full_name': full_name, 'email': email, 'address': address, 'phone': phone, 'msg_id': query.message.message_id, 'from_profile': True})
    ...
    async def handle_checkout_input(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        if state['step'] == 'waiting_full_name':
            min_words = 3 if SHIPPING_MODE == 'UKRAINE' else 2
            if len(text.split()) < min_words: return await send_err('err_invalid_name', "confirm_details_back" if is_edit else "cart")
            state['full_name'] = text

        elif state['step'] == 'waiting_email':
            if not re.fullmatch(r"[^@\s]+@[^@\s]+\.[^@\s]+", text): return await send_err('err_invalid_email', "confirm_details_back" if is_edit else "back_to_name")
            state['email'] = text
            ...
            if SHIPPING_MODE == 'UKRAINE':
                ...
                is_valid = len(tokens) >= 4 and bool(re.search(r'\d', text))
                err_key = 'err_invalid_address_int'
            if not is_valid: return await send_err(err_key, "confirm_details_back" if is_edit else "back_to_email")
            state['address'] = text
    ...
    async def show_order_summary(self, context, chat_id, user_id):
        ...
        try:
            if 'msg_id' in state: await context.bot.edit_message_text(chat_id=chat_id, message_id=state['msg_id'], text=summary_text, reply_markup=keyboard, parse_mode="HTML")
    ...
    async def handle_checkout_confirm(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        if query.data == "confirm_details": await self.send_payment_keyboard(context, query.message.chat_id, user_id)
        elif query.data == "confirm_details_back": await self.show_order_summary(context, query.message.chat_id, user_id)
        elif query.data == "back_to_payment":
            try: await query.message.delete()
            except: pass
    ...
    async def precheckout_callback(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        async with self.conn.execute("SELECT c.quantity, p.name, p.stock FROM cart c JOIN products p ON c.product_id = p.id WHERE c.user_id = ?", (query.from_user.id,)) as cursor:
            for qty, name, stock in await cursor.fetchall():
                if stock < qty: return await query.answer(ok=False, error_message=self.get_text('stock_out_error', name=name))
    ...
    async def successful_payment_callback(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        await self.finalize_order(update, context, self.get_text('method_online_card'), update.message.successful_payment.total_amount / 100)

    async def finalize_order(self, update: Update, context: ContextTypes.DEFAULT_TYPE, payment_method, pre_calc_total=None):
        user_id = update.effective_user.id
        chat_id = update.effective_chat.id
        query = getattr(update, "callback_query", None)
        ...
        if not result: return await context.bot.send_message(chat_id=chat_id, text=self.get_text('order_failed_cart_empty'))

        order_id, products_list, total_amount, promo_code_used = result

        details = self.generate_receipt(order_id, state.get('full_name', update.effective_user.full_name), state.get('email', '—'), state.get('phone', '—'), state.get('address', '—'), payment_method, products_list, total_amount, datetime.now(ZoneInfo(BOT_TIMEZONE)).strftime('%d.%m.%Y %H:%M'), receipt_format='html')
        final_text = f"{STORE_MESSAGES[SHIPPING_MODE]['order_success'].format(order_id=order_id)}\n\n{details}"

        if promo_code_used: final_text += self.get_text('order_promo_info', code=promo_code_used)

        kb = InlineKeyboardMarkup([[InlineKeyboardButton(self.get_text('main_menu_button'), callback_data="main_menu")]])

        if query:
            try: await query.edit_message_text(text=final_text, reply_markup=kb, parse_mode=ParseMode.HTML)
            except Exception:
                try: await query.message.delete()
                except Exception: pass
                await context.bot.send_message(chat_id=chat_id, text=final_text, reply_markup=kb, parse_mode=ParseMode.HTML)
        else: await context.bot.send_message(chat_id=chat_id, text=final_text, reply_markup=kb, parse_mode=ParseMode.HTML)
        self.user_states.pop(user_id, None)
    ...
    async def create_order(self, update: Update, context: ContextTypes.DEFAULT_TYPE, send_message=False, payment_method_override=None):
        ...
        async with self.conn.execute('SELECT c.id, p.name, p.price, c.quantity, p.emoji, c.selected_options, p.variants, p.id, p.stock FROM cart c JOIN products p ON c.product_id = p.id WHERE c.user_id = ?', (user_id,)) as cursor:
            cart_items = await cursor.fetchall()
        ...
        for cart_id, name, base_price, quantity, emoji, opts_json, variants_json, product_id, p_stock in cart_items:
            ...
            products_list.append({"product_id": product_id, "name": name, "price": real_price, "quantity": quantity, "total": item_total, "emoji": emoji, "selected_options": sel_opts})

            async with self.conn.execute("SELECT stock, variants FROM products WHERE id = ?", (product_id,)) as cursor:
                fresh_prod = await cursor.fetchone()
            ...
            if fresh_prod:
                ...
                await self.conn.execute("UPDATE products SET stock = ?, variants = ? WHERE id = ?", (max(0, curr_stock - quantity), new_variants_json, product_id))
        ...
        if active_promo:
            ...
            await self.conn.execute("UPDATE promocodes SET current_uses = current_uses + 1 WHERE code = ?", (promo_code,))
            await self.conn.execute("INSERT INTO used_promocodes (user_id, code) VALUES (?, ?)", (user_id, promo_code))
        ...
        async with self.conn.execute("INSERT INTO orders (user_id, user_name, full_name, products, total_amount, phone, address, payment_method, email, promo_code) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)", (user_id, update.effective_user.full_name, full_name, json.dumps(products_list, ensure_ascii=False), total_amount, phone, address, payment_method, email, promo_code_used)) as cursor:
            order_id = cursor.lastrowid

        await self.conn.execute("DELETE FROM cart WHERE user_id = ?", (user_id,))
        await self.conn.commit()

        if send_message: await self.notify_admin_new_order(context, order_id, full_name, email, phone, address, payment_method, products_list, total_amount, promo_code_used)
        return order_id, products_list, total_amount, promo_code_used

    async def notify_admin_new_order(self, context, order_id, full_name, email, phone, address, payment_method, products_list, total_amount, promo_code_used=None):
        region_header = self.get_text('new_order_notification_ukraine' if SHIPPING_MODE == 'UKRAINE' else 'new_order_notification_international')
        address_label = self.get_text('delivery_notification_ukraine' if SHIPPING_MODE == 'UKRAINE' else 'delivery_notification_international')
        pay_label = self.get_text('payment_notification_ukraine' if SHIPPING_MODE == 'UKRAINE' else 'payment_notification_international')
        ...
        text = self.get_text('admin_new_order_notification', region_header=region_header, order_id=order_id, full_name=full_name, email=email, phone=phone, address_label=address_label, address=address, pay_label=pay_label, payment_method=payment_method, items_str=items_str, total_amount=total_amount)
        if promo_code_used: text += self.get_text('order_promo_info', code=promo_code_used)

        for admin_id in ADMIN_IDS:
            try: await context.bot.send_message(chat_id=admin_id, text=text, parse_mode=ParseMode.HTML)
            except Exception as e: logger.error(self.get_text('failed_to_notify_admin', admin=admin_id, e=e))
    ...
    async def show_my_orders(self, update: Update, context: ContextTypes.DEFAULT_TYPE, page: int = 0):
        ...
        async with self.conn.execute("SELECT COUNT(*) as total FROM orders WHERE user_id = ?", (user_id,)) as cursor:
            total_orders = (await cursor.fetchone())["total"]

        if total_orders == 0:
            return await query.edit_message_text(self.get_text('no_orders_yet'), reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton(self.get_text('main_menu_button'), callback_data="main_menu")]]), parse_mode="HTML")

        per_page = ITEMS_PER_PAGE
        total_pages = (total_orders - 1) // per_page + 1

        async with self.conn.execute('SELECT id, total_amount, status, created_at, products FROM orders WHERE user_id = ? ORDER BY id DESC LIMIT ? OFFSET ?', (user_id, per_page, page * per_page)) as cursor:
            orders = await cursor.fetchall()

        text = self.get_text('your_orders_header', page=page + 1, total_pages=total_pages)
        keyboard = []
        status_map = {'pending': self.get_text('status_pending'), 'confirmed': self.get_text('status_confirmed'), 'shipped': self.get_text('status_shipped'), 'delivered': self.get_text('status_delivered'), 'cancelled': self.get_text('status_cancelled')}
        ...
        for order in orders:
            ...
            text += self.get_text('order_summary_line', order_id=order['id'], products_str=self.escape_html(products_str), total_amount=order['total_amount'], status_text=display_status, date=self.format_date(order['created_at']))
            keyboard.append([InlineKeyboardButton(self.get_text('details_button', order_id=order['id']), callback_data=f"order_details_{order['id']}_{page}")])

        nav = []
        if page > 0: nav.append(InlineKeyboardButton(self.get_text('prev_button'), callback_data=f"my_orders_page_{page - 1}"))
        if page + 1 < total_pages: nav.append(InlineKeyboardButton(self.get_text('next_button'), callback_data=f"my_orders_page_{page + 1}"))
        if nav: keyboard.append(nav)
        keyboard.append([InlineKeyboardButton(self.get_text('main_menu_button'), callback_data="main_menu")])
    ...
    async def show_order_details(self, update: Update, context: ContextTypes.DEFAULT_TYPE, order_id=None, origin_page=0):
        if await self.check_user_blocked(update, context): return
        query = getattr(update, "callback_query", None)
        user_id = update.effective_user.id
        ...
        if int(user_id) in ADMIN_IDS:
            ...
            async with self.conn.execute("SELECT * FROM orders WHERE id = ? AND user_id = ?", (order_id, user_id)) as cursor:
                order = await cursor.fetchone()
        ...
        try:
            for p in json.loads(order["products"]):
                opts_str = f" ({', '.join([str(v) for v in p.get('selected_options', {}).values()])})" if p.get('selected_options') else ""
            ...
            products_text = "\n".join([f"📦 {self.escape_html(line)}" for line in str(order["products"]).split('\n') if line.strip()]) if order["products"] else self.get_text('items_info_unavailable')

        status_map = {'pending': self.get_text('status_pending'), 'confirmed': self.get_text('status_confirmed'), 'shipped': self.get_text('status_shipped'), 'delivered': self.get_text('status_delivered'), 'cancelled': self.get_text('status_cancelled')}
        display_status = status_map.get(order['status'], order['status'])

        text = self.get_text('order_details_text', order_id=order['id'], user_name=self.escape_html(order['user_name']), email=self.escape_html(order['email'] or self.get_text('not_specified_dash')), phone=self.escape_html(order['phone'] or self.get_text('not_specified_dash')), address=self.escape_html(order['address']), payment_method=self.escape_html(order['payment_method'] or self.get_text('not_specified_dash')), products_text=products_text, total_amount=order['total_amount'], status_display=display_status, date=self.format_date(order['created_at']))

        promo_val = order['promo_code'] if 'promo_code' in order.keys() else None
        if promo_val: text += self.get_text('order_promo_info', code=promo_val)

        keyboard = []
        if int(user_id) in ADMIN_IDS:
            if order['status'] not in ('cancelled', 'delivered'):
                keyboard.extend([[InlineKeyboardButton(self.get_text('confirm_button'), callback_data=f"admin_confirm_{order['id']}_{origin_page}"), InlineKeyboardButton(self.get_text('sent_button'), callback_data=f"admin_ship_{order['id']}_{origin_page}")], [InlineKeyboardButton(self.get_text('delivered_button'), callback_data=f"admin_deliver_{order['id']}_{origin_page}"), InlineKeyboardButton(self.get_text('cancel_button_2'), callback_data=f"admin_cancel_{order['id']}_{origin_page}")]])
            keyboard.append([InlineKeyboardButton(self.get_text('back_to_all_orders_button'), callback_data=f"admin_all_orders_page_{origin_page}")])
        else:
            if order['status'] not in ('cancelled', 'delivered'): keyboard.append([InlineKeyboardButton(self.get_text('cancel_order_button_3'), callback_data=f"user_cancel_{order['id']}")])
        ...
        else: await context.bot.send_message(update.effective_chat.id, text, reply_markup=InlineKeyboardMarkup(keyboard), parse_mode="HTML")
    ...
    async def admin_broadcast_prompt(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        query = update.callback_query
        self.user_states[update.effective_user.id] = {'step': 'waiting_broadcast_message', 'msg_id': query.message.message_id}
        await query.edit_message_text(self.get_text('admin_broadcast_prompt'), reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton(self.get_text('cancel_button'), callback_data="admin_panel")]]), parse_mode="HTML")
    ...
    async def handle_broadcast_input(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        if 'msg_id' in state:
            try: await context.bot.edit_message_text(chat_id=chat_id, message_id=state['msg_id'], text=self.get_text('admin_broadcast_started'), parse_mode="HTML")
            except Exception: pass
        else:
        ...
        try: await context.bot.edit_message_text(chat_id=chat_id, message_id=state['msg_id'], text=finish_text, reply_markup=kb, parse_mode="HTML")
        except Exception: await context.bot.send_message(chat_id=chat_id, text=finish_text, reply_markup=kb, parse_mode="HTML")
        self.user_states.pop(user_id, None)
    ...
    async def admin_promo_menu(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        async with self.conn.execute("SELECT code, discount, max_uses, current_uses, is_reusable FROM promocodes") as cursor:
            promos = await cursor.fetchall()

        promo_list = ""
        if not promos: promo_list = self.get_text('promo_no_active')
        else:
            for code, discount, max_uses, current_uses, is_reusable in promos:
                left = max_uses - current_uses
                is_reusable = is_reusable or 0
                reusable_text = self.get_text('promo_reusable_multi') if is_reusable else self.get_text('promo_reusable_once')
                promo_list += self.get_text('promo_item_detailed', code=code, discount=discount, left=left, reusable_text=reusable_text) + "\n"

        keyboard = [
            [InlineKeyboardButton(self.get_text('admin_promo_add_btn'), callback_data="admin_promo_add_prompt")],
            [InlineKeyboardButton(self.get_text('admin_promo_del_btn'), callback_data="admin_promo_del_prompt")] if promos else [],
            [InlineKeyboardButton(self.get_text('back_to_admin_panel_button'), callback_data="admin_panel")]
        ]

        try: await update.callback_query.edit_message_text(self.get_text('admin_promo_menu', promo_list=promo_list), reply_markup=InlineKeyboardMarkup(keyboard), parse_mode="HTML")
        except: pass

    async def admin_promo_add_prompt(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        if int(update.effective_user.id) not in ADMIN_IDS: return
        query = update.callback_query
        self.user_states[update.effective_user.id] = {'step': 'waiting_promo_code_name', 'msg_id': query.message.message_id}
        await query.edit_message_text(self.get_text('admin_promo_ask_code'), reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton(self.get_text('cancel_button'), callback_data="admin_promo_menu")]]), parse_mode="HTML")

    async def admin_promo_del_prompt(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        if int(update.effective_user.id) not in ADMIN_IDS: return

        async with self.conn.execute("SELECT id, code FROM promocodes") as cursor:
            promos = await cursor.fetchall()

        if not promos: return await self.admin_promo_menu(update, context)
        ...
        await update.callback_query.edit_message_text(self.get_text('admin_promo_del_select'), reply_markup=InlineKeyboardMarkup(kb), parse_mode="HTML")
    ...
    async def admin_delete_promo_action(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        async with self.conn.execute("SELECT code FROM promocodes WHERE id = ?", (pid,)) as cursor:
            row = await cursor.fetchone()

        if row:
            code = row[0]
            await self.conn.execute("DELETE FROM promocodes WHERE id = ?", (pid,))
            await self.conn.commit()
            await query.answer(self.get_text('admin_promo_deleted', code=code))
        else: await query.answer(self.get_text('admin_promo_not_found'))
    ...
    async def handle_promo_input(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        if step == 'waiting_promo_code_name':
            ...
            m = await context.bot.send_message(chat_id=chat_id, text=self.get_text('admin_promo_ask_discount', code=state['promo_code']), reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton(self.get_text('cancel_button'), callback_data="admin_promo_menu")]]), parse_mode="HTML")
            state['msg_id'] = m.message_id
            ...
            try:
                ...
                m = await context.bot.send_message(chat_id=chat_id, text=self.get_text('admin_promo_ask_uses'), reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton(self.get_text('cancel_button'), callback_data="admin_promo_menu")]]), parse_mode="HTML")
                state['msg_id'] = m.message_id
            except ValueError:
                m = await context.bot.send_message(chat_id=chat_id, text=self.get_text('err_invalid_discount'), reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton(self.get_text('cancel_button'), callback_data="admin_promo_menu")]]), parse_mode="HTML")
                state['msg_id'] = m.message_id
            ...
            try:
                ...
                kb = [[InlineKeyboardButton(self.get_text('btn_promo_once'), callback_data="admin_promo_reusable_0")], [InlineKeyboardButton(self.get_text('btn_promo_multi'), callback_data="admin_promo_reusable_1")], [InlineKeyboardButton(self.get_text('cancel_button'), callback_data="admin_promo_menu")]]
                m = await context.bot.send_message(chat_id=chat_id, text=self.get_text('admin_promo_ask_reusable'), reply_markup=InlineKeyboardMarkup(kb), parse_mode="HTML")
                state['msg_id'] = m.message_id
            except ValueError:
                m = await context.bot.send_message(chat_id=chat_id, text=self.get_text('err_invalid_uses'), reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton(self.get_text('cancel_button'), callback_data="admin_promo_menu")]]), parse_mode="HTML")
                state['msg_id'] = m.message_id
    ...
    async def admin_promo_set_reusable(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        await self.conn.execute("INSERT OR REPLACE INTO promocodes (code, discount, max_uses, current_uses, is_reusable) VALUES (?, ?, ?, 0, ?)", (code, discount, max_uses, is_reusable))
        await self.conn.commit()

        await query.edit_message_text(text=self.get_text('admin_promo_added', code=code, discount=discount), reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton(self.get_text('back_button_3'), callback_data="admin_promo_menu")]]), parse_mode="HTML")
        self.user_states.pop(user_id, None)
    ...
    async def ask_promo_code(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        msg = await query.edit_message_text(self.get_text('ask_promo_code'), reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton(self.get_text('cancel_button'), callback_data="cart")]]), parse_mode="HTML")
    ...
    async def handle_user_promo_input(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        async with self.conn.execute("SELECT discount, max_uses, current_uses, is_reusable FROM promocodes WHERE code = ?", (text,)) as cursor:
            res = await cursor.fetchone()
        ...
        if res:
            ...
            if current_uses >= max_uses:
                ...
                if not is_reusable:
                    async with self.conn.execute("SELECT 1 FROM used_promocodes WHERE user_id = ? AND code = ?", (user_id, text)) as cursor2:
    ...
    async def admin_statistics(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        async with self.conn.execute("SELECT COUNT(*), (SELECT COUNT(*) FROM orders WHERE status = 'pending'), (SELECT SUM(total_amount) FROM orders WHERE status IN ('confirmed', 'shipped', 'delivered')), (SELECT COUNT(*) FROM users), (SELECT COUNT(DISTINCT user_id) FROM orders) FROM orders") as cursor:
            total_orders, pending_orders, total_revenue, total_users, active_buyers = await cursor.fetchone()

        async with self.conn.execute("SELECT products FROM orders WHERE status IN ('confirmed', 'shipped', 'delivered')") as cursor:
            product_rows = await cursor.fetchall()

        product_sales = {}
        for (products_json,) in product_rows:
            try:
                for item in json.loads(products_json): product_sales[item.get('name', self.get_text('unknown'))] = product_sales.get(item.get('name', self.get_text('unknown')), 0) + item.get('quantity', 0)
            except: pass

        sorted_sales = sorted(product_sales.items(), key=lambda x: x[1], reverse=True)

        pcs_str = self.get_text('unit_pcs')
        top_text = "\n".join([f"🔥 {name}: {qty} {pcs_str}" for name, qty in sorted_sales[:5]]) if sorted_sales[:5] else self.get_text('no_data')
        bottom_text = "\n".join([f"🧊 {name}: {qty} {pcs_str}" for name, qty in sorted_sales[-5:]]) if sorted_sales[-5:] else self.get_text('no_data')

        text = self.get_text('admin_stats_text', total_revenue=total_revenue or 0, total_orders=total_orders, pending_orders=pending_orders, total_users=total_users, active_buyers=active_buyers, top_text=top_text, bottom_text=bottom_text)

        await update.callback_query.edit_message_text(text, reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton(self.get_text('back_to_admin_panel_button'), callback_data="admin_panel")]]), parse_mode="HTML")
    ...
    async def admin_revenue(self, update: Update, context: ContextTypes.DEFAULT_TYPE, period: str = "all"):
        ...
        if period == "today": period_sql, label = " AND created_at >= date('now', 'localtime')", self.get_text('today')
        elif period == "week": period_sql, label = " AND created_at >= date('now', '-7 days')", self.get_text('last_7_days')
        elif period == "month": period_sql, label = " AND created_at >= date('now', '-30 days')", self.get_text('last_30_days')

        async with self.conn.execute(f"SELECT SUM(total_amount), COUNT(id) FROM orders WHERE status IN ('confirmed', 'shipped', 'delivered'){period_sql}") as cursor:
            total_rev, total_orders = await cursor.fetchone()
        async with self.conn.execute(f"SELECT SUM(total_amount) FROM orders WHERE status = 'pending'{period_sql}") as cursor:
            res = await cursor.fetchone()
            pending_rev = res[0] or 0 if res else 0

        text = self.get_text('financial_report', label=label.upper(), total_rev=total_rev or 0, avg_check=round((total_rev or 0) / (total_orders or 1), 2), total_orders=total_orders or 0, pending_rev=pending_rev)
        keyboard = [[InlineKeyboardButton(self.get_text('today_button'), callback_data="rev_today"), InlineKeyboardButton(self.get_text('week_button'), callback_data="rev_week"), InlineKeyboardButton(self.get_text('month_button'), callback_data="rev_month")], [InlineKeyboardButton(self.get_text('all_time_button'), callback_data="rev_all")], [InlineKeyboardButton(self.get_text('back_to_admin_panel_button'), callback_data="admin_panel")]]

        try: await update.callback_query.edit_message_text(text, reply_markup=InlineKeyboardMarkup(keyboard), parse_mode="HTML")
        except: pass
    ...
    async def admin_user_management(self, update: Update, context: ContextTypes.DEFAULT_TYPE, page: int = 0):
        ...
        async with self.conn.execute("SELECT user_id, blocked FROM users LIMIT ? OFFSET ?", (ADMIN_ITEMS_PER_PAGE, page * ADMIN_ITEMS_PER_PAGE)) as cursor:
            users_data = await cursor.fetchall()

        keyboard = []
        for user_id, blocked in users_data:
            try:
                chat = await context.bot.get_chat(user_id)
                user_display = f"@{chat.username}" if chat.username else (chat.first_name or self.get_text('user_id', user_id=user_id))
            except:
                user_display = self.get_text('user_id', user_id=user_id)

            keyboard.append([InlineKeyboardButton(self.get_text('user_display', user_display=user_display), callback_data="noop"), InlineKeyboardButton(self.get_text('unblock_button') if blocked else self.get_text('block_button'), callback_data=f"admin_user_block_{user_id}_{0 if blocked else 1}")])

        nav = []
        if page > 0: nav.append(InlineKeyboardButton(self.get_text('prev_button'), callback_data=f"admin_user_page_{page - 1}"))
        if page + 1 < total_pages: nav.append(InlineKeyboardButton(self.get_text('next_button'), callback_data=f"admin_user_page_{page + 1}"))
        if nav: keyboard.append(nav)
        keyboard.append([InlineKeyboardButton(self.get_text('admin_panel_button_2'), callback_data="admin_panel")])

        try: await update.callback_query.edit_message_text(self.get_text('user_management_header', page=page + 1, total_pages=total_pages, total_users=total_users), reply_markup=InlineKeyboardMarkup(keyboard), parse_mode=ParseMode.MARKDOWN)
        except: pass
    ...
    async def admin_user_block(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        await self.conn.execute("UPDATE users SET blocked = ? WHERE user_id = ?", (int(block_status), int(user_id)))
        await self.conn.commit()
    ...
    async def admin_all_orders(self, update: Update, context: ContextTypes.DEFAULT_TYPE, page: int = 0):
        query = update.callback_query
        if int(update.effective_user.id) not in ADMIN_IDS: return await query.answer(self.get_text('access_denied_not_admin'), show_alert=True)

        async with self.conn.execute("SELECT COUNT(*) as total FROM orders") as cursor:
            res = await cursor.fetchone()
            total_orders = res["total"] if res else 0

        total_pages = max(1, (total_orders - 1) // ADMIN_ITEMS_PER_PAGE + 1) if total_orders else 1

        async with self.conn.execute('SELECT id, user_name, total_amount, status, created_at FROM orders ORDER BY id DESC LIMIT ? OFFSET ?', (ADMIN_ITEMS_PER_PAGE, page * ADMIN_ITEMS_PER_PAGE)) as cursor:
            orders = await cursor.fetchall()

        if not orders: return await query.edit_message_text(self.get_text('no_orders_in_db'), reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton(self.get_text('back_button_2'), callback_data="admin_panel")]]))
        ...
        for o in orders:
            ...
            keyboard.append([InlineKeyboardButton(self.get_text('details_button_2', order_id=o['id']), callback_data=f"order_details_{o['id']}_{page}")])

        nav = []
        if page > 0: nav.append(InlineKeyboardButton(self.get_text('prev_button'), callback_data=f"admin_all_orders_page_{page - 1}"))
        if page + 1 < total_pages: nav.append(InlineKeyboardButton(self.get_text('next_button'), callback_data=f"admin_all_orders_page_{page + 1}"))
        if nav: keyboard.append(nav)
    ...
    async def admin_order_status_change(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        new_status = {"confirm": "confirmed", "ship": "shipped", "deliver": "delivered", "cancel": "cancelled"}.get(match.group(1))
        order_id, origin_page = int(match.group(2)), int(match.group(3)) if match.group(3) else 0

        if new_status == 'cancelled':
            async with self.conn.execute("SELECT status FROM orders WHERE id = ?", (order_id,)) as cursor:
                res = await cursor.fetchone()
                if res and res[0] != 'cancelled': await self.restore_stock(order_id)

        await self.conn.execute("UPDATE orders SET status = ? WHERE id = ?", (new_status, order_id))
        await self.conn.commit()
        ...
        try:
            async with self.conn.execute("SELECT user_id FROM orders WHERE id = ?", (order_id,)) as cursor:
                row = await cursor.fetchone()
            if row:
                msg_text = self.get_text('order_update_notification_ukraine' if SHIPPING_MODE == 'UKRAINE' else 'order_update_notification_international', order_id=order_id, display_status=status_localized)
    ...
    async def admin_categories_menu(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        await self.cleanup_albums(update.effective_user.id, context, update.effective_chat.id)
        query = update.callback_query
        page = int(query.data.split("_")[-1]) if query and query.data.startswith("admin_cat_page_") and query.data.split("_")[-1].isdigit() else 1

        async with self.conn.execute("SELECT COUNT(DISTINCT category) FROM products WHERE is_active = 1") as cursor:
            res = await cursor.fetchone()
            total_cat = res[0] if res else 0

        total_pages = max(1, (total_cat + ITEMS_PER_PAGE - 1) // ITEMS_PER_PAGE)
        page = max(1, min(page, total_pages))

        async with self.conn.execute(f"SELECT DISTINCT category FROM products WHERE is_active = 1 ORDER BY category ASC LIMIT {ITEMS_PER_PAGE} OFFSET ?", ((page - 1) * ITEMS_PER_PAGE,)) as cursor:
            categories = await cursor.fetchall()

        text = self.get_text('product_management_header') + (self.get_text('page_indicator_2', page=page, total_pages=total_pages) if total_pages > 1 else "") + self.get_text('select_category_to_edit')

        keyboard = []
        for (cat_name,) in categories:
            async with self.conn.execute("SELECT COUNT(*) FROM products WHERE category = ? AND is_active = 1", (cat_name,)) as cursor2:
                cat_count = (await cursor2.fetchone())[0]
            keyboard.append([InlineKeyboardButton(self.get_text('category_button_count', cat_name=cat_name, count=cat_count), callback_data=f"admin_list_cat_{cat_name}_1")])

        nav = []
        if page > 1: nav.append(InlineKeyboardButton(self.get_text('prev_button'), callback_data=f"admin_cat_page_{page - 1}"))
        if page < total_pages: nav.append(InlineKeyboardButton(self.get_text('next_button'), callback_data=f"admin_cat_page_{page + 1}"))
        if nav: keyboard.append(nav)

        keyboard.extend([[InlineKeyboardButton(self.get_text('add_product_button'), callback_data="admin_add_product")], [InlineKeyboardButton(self.get_text('back_to_admin_panel_button'), callback_data="admin_panel")]])
    ...
    async def admin_products_list(self, update: Update, context: ContextTypes.DEFAULT_TYPE, category_override=None):
        await self.cleanup_albums(update.effective_user.id, context, update.effective_chat.id)
        query = update.callback_query
        if category_override: category, page = category_override, 1
        else:
            try:
                parts = query.data.split("_")
                page, category = int(parts[-1]), "_".join(parts[3:-1])
            except: return await query.answer(self.get_text('error_parsing_category'))
        ...
        total_pages = (total_items + ITEMS_PER_PAGE - 1) // ITEMS_PER_PAGE
        async with self.conn.execute("SELECT id, name, stock FROM products WHERE category = ? AND is_active = 1 LIMIT ? OFFSET ?", (category, ITEMS_PER_PAGE, (page - 1) * ITEMS_PER_PAGE)) as cursor:
            products = await cursor.fetchall()

        text = self.get_text('category_header_2', category=category, page=page, total_pages=total_pages)
        keyboard = [[InlineKeyboardButton(f"{'✅' if p_stock > 0 else '❌'} {p_name}", callback_data=f"admin_prod_{p_id}_{page}")] for p_id, p_name, p_stock in products]

        nav = []
        if page > 1: nav.append(InlineKeyboardButton(self.get_text('prev_button'), callback_data=f"admin_list_cat_{category}_{page - 1}"))
    ...
    async def admin_product_menu(self, update: Update, context: ContextTypes.DEFAULT_TYPE, product_id_override=None):
        ...
        try:
            if product_id_override: product_id = int(product_id_override)
            elif query:
                parts = query.data.split("_")
                product_id = int(parts[2])
                origin_page = int(parts[3]) if len(parts) >= 4 else 1
            else: return
        except: return
                ...
                for key, val in v_data.items():
                    if isinstance(val, dict):
                        ...
                        for opt, info in sorted(val.items(), key=lambda x: x[0]):
                            ...
                            price_str = f" ({float(info['price'])}{CURRENCY_SYMBOL})" if isinstance(info, dict) and 'price' in info else ""
                            if isinstance(info, dict) and 'price' in info: all_prices.append(float(info['price']))
                            elif product['price'] > 0: all_prices.append(product['price'])
        ...
        keyboard = [
            [InlineKeyboardButton(self.get_text('edit_name_button_3'), callback_data=f"admin_edit_field_name_{product_id}"), InlineKeyboardButton(self.get_text('edit_desc_button'), callback_data=f"admin_edit_field_description_{product_id}")],
            [InlineKeyboardButton(self.get_text('edit_price_button'), callback_data=f"admin_edit_field_price_{product_id}"), InlineKeyboardButton(self.get_text('edit_stock_button'), callback_data=f"admin_edit_field_stock_{product_id}")],
            [InlineKeyboardButton(self.get_text('edit_category_button'), callback_data=f"admin_edit_field_category_{product_id}"), InlineKeyboardButton(self.get_text('edit_emoji_button'), callback_data=f"admin_edit_field_emoji_{product_id}")],
            [InlineKeyboardButton(self.get_text('edit_image_button'), callback_data=f"admin_image_menu_{product_id}"), InlineKeyboardButton(self.get_text('edit_variants_button'), callback_data=f"admin_edit_field_variants_{product_id}")],
            [InlineKeyboardButton(self.get_text('delete_product_button'), callback_data=f"admin_delete_product_confirm_{product_id}")],
            [InlineKeyboardButton(self.get_text('back_to_list_button_2'), callback_data=f"admin_list_cat_{product['category']}_{origin_page}")]
        ]
        ...
        if len(images) > 1:
            media_group = [InputMediaPhoto(media=img, caption=text if i == 0 else "", parse_mode="MARKDOWN") for i, img in enumerate(images[:10])]
            ...
            await context.bot.send_message(chat_id=update.effective_chat.id, text=self.get_text('choose_action'), reply_markup=InlineKeyboardMarkup(keyboard), parse_mode="HTML")
        elif len(images) == 1:
            await context.bot.send_photo(chat_id=update.effective_chat.id, photo=images[0], caption=text, reply_markup=InlineKeyboardMarkup(keyboard), parse_mode=ParseMode.MARKDOWN)
        else:
            await context.bot.send_message(chat_id=update.effective_chat.id, text=text, reply_markup=InlineKeyboardMarkup(keyboard), parse_mode=ParseMode.MARKDOWN)
    ...
    async def admin_add_product(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        msg = await update.callback_query.edit_message_text(self.get_text('adding_new_product_name'), reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton(self.get_text('cancel_button'), callback_data="admin_products")]]), parse_mode=ParseMode.MARKDOWN)
        self.user_states[update.effective_user.id]['msg_id'] = msg.message_id
    ...
    async def admin_edit_field(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        if field in ['price', 'stock'] and product['variants']:
            try:
                if json.loads(product['variants']): return await query.answer(self.get_text('alert_has_variants'), show_alert=True)
            except: pass
        ...
        if field == "variants":
            ...
            display_name = {"name": self.get_text('prod_name'), "description": self.get_text('desc'), "price": self.get_text('price'), "stock": self.get_text('stock'), "emoji": self.get_text('emoji'), "category": self.get_text('category')}.get(field, field.capitalize())
        ...
        self.user_states[update.effective_user.id] = {'step': 'edit_product_field', 'product_id': product_id, 'field': field}
        await query.message.delete()

        reply_kb = await self.get_existing_categories_keyboard(product_id=product_id) if field == "category" else InlineKeyboardMarkup([[InlineKeyboardButton(self.get_text('cancel_button'), callback_data=f"admin_prod_{product_id}")]])
        sent_msg = await context.bot.send_message(chat_id=query.message.chat_id, text=msg_text, reply_markup=reply_kb, parse_mode="HTML")
    ...
    async def admin_image_menu(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        images_list = []
        if 'images' in product.keys() and product['images']:
            try: images_list = json.loads(product['images'])
            except: pass
        if not images_list and product['image_url']: images_list = [product['image_url']]

        photo_count = len(images_list)
        if photo_count > 0: status_text = self.get_text('admin_img_status_count', count=photo_count)
        else: status_text = self.get_text('admin_img_status_none')

        text = self.get_text('product_image_management', product_name=product['name'], status=status_text)

        keyboard = []
        if photo_count < 10: keyboard.append([InlineKeyboardButton(self.get_text('add_photo_button') if photo_count == 0 else self.get_text('add_more_photos_button'), callback_data=f"admin_image_set_{product_id}")])
        if photo_count > 0: keyboard.append([InlineKeyboardButton(self.get_text('delete_all_photos_button'), callback_data=f"admin_image_delete_{product_id}")])
    ...
    async def admin_image_set_prompt(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        msg = await context.bot.send_message(chat_id=query.message.chat_id, text=self.get_text('send_product_image'), reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton(self.get_text('cancel_button_3'), callback_data=f"admin_image_menu_{match.group(1)}")]]), parse_mode=ParseMode.MARKDOWN)
    ...
    async def admin_image_delete(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        product_id = int(match.group(1))
        await self.conn.execute("UPDATE products SET image_url = NULL, images = NULL WHERE id = ?", (product_id,))
        await self.conn.commit()
    ...
    async def admin_delete_product(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        async with self.conn.execute("SELECT name FROM products WHERE id = ?", (product_id,)) as cursor:
            row = await cursor.fetchone()
        ...
        await context.bot.send_message(chat_id=query.message.chat_id, text=self.get_text('confirm_delete_product', name=row[0]), reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton(self.get_text('yes_delete_button'), callback_data=f"admin_delete_product_confirm_{product_id}")], [InlineKeyboardButton(self.get_text('cancel_button'), callback_data="admin_products")]]), parse_mode=ParseMode.MARKDOWN)
    ...
    async def admin_delete_product_confirm(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        async with self.conn.execute("SELECT name, category FROM products WHERE id = ?", (product_id,)) as cursor:
            row = await cursor.fetchone()
        ...
        await self.conn.execute("UPDATE products SET is_active = 0 WHERE id = ?", (product_id,))
        await self.conn.commit()
        ...
        if user_id in self.user_states:
            ...
            if step == 'waiting_search_query':
                ...
                try:
                    ...
                    if 'msg_id' in self.user_states[user_id]:
                        await context.bot.delete_message(chat_id=update.message.chat_id, message_id=self.user_states[user_id]['msg_id'])
                except: pass
    ...
    async def handle_edit_field_input(self, update, context, state, input_value, msg):
        ...
        async def send_error(text):
            new_msg = await context.bot.send_message(chat_id=chat_id, text=text, reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton(self.get_text('cancel_button'), callback_data=f"admin_prod_{product_id}")]]), parse_mode="HTML")
            state['msg_id'] = new_msg.message_id

        value, new_total_stock = input_value, None

        if field in ['price', 'stock']:
            try:
                value = float(str(input_value).replace('$', '').replace(' ', '').replace(',', '.').strip()) if field == 'price' else int(str(input_value).replace(' ', '').strip())
                if value < 0: raise ValueError()
            except ValueError:
                return await send_error(self.get_text('err_invalid_number', val=input_value, ex_val=self.get_text('ex_price_val') if field == 'price' else self.get_text('ex_stock_val')))
        elif field == 'variants':
            try:
                if ":" in str(input_value):
                    ...
                    for opt in options_part.split(","):
                        ...
                        v_data[v_type_part.strip()][parts[0].strip()] = {"qty": qty, "price": float(parts[2].strip()) if len(parts) > 2 else 0}
        ...
        try:
            if new_total_stock is not None:
                await self.conn.execute("UPDATE products SET variants = ?, stock = ? WHERE id = ?", (value, new_total_stock, product_id))
            else:
        ...
        display_field = {"name": self.get_text('prod_name'), "description": self.get_text('desc'), "price": self.get_text('price'), "stock": self.get_text('stock'), "category": self.get_text('category'), "emoji": self.get_text('emoji'), "variants": self.get_text('variants')}.get(field, str(field).capitalize())
        ...
        await context.bot.send_message(chat_id=chat_id, text=success_msg, reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton(self.get_text('back_button_3'), callback_data=f"admin_prod_{product_id}")]]), parse_mode="HTML")
    ...
    async def handle_admin_product_input(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        input_value = update.message.photo[-1].file_id if update.message.photo else (update.message.text.strip() if update.message.text else "")
        ...
        if step == 'waiting_product_image':
            ...
            if img or input_value == '-':
                images_list = []
                if img:
                    async with self.conn.execute("SELECT images, image_url FROM products WHERE id = ?", (state.get('product_id'),)) as cursor:
                        row = await cursor.fetchone()
                    if row:
                        if row[0]:
                            try: images_list = json.loads(row[0])
                            except: pass
                        elif row[1]:
                            images_list = [row[1]]
        ...
        if step == 'add_product_name':
            ...
            try:
                ...
                m = await context.bot.send_message(chat_id=chat_id, text=self.get_text('err_invalid_number', val=input_value, ex_val=self.get_text('ex_price_val')), reply_markup=cancel_kb, parse_mode="HTML")
            state['msg_id'] = m.message_id
            ...
            try:
                ...
                reply_kb = await self.get_existing_categories_keyboard()
                m = await context.bot.send_message(chat_id=chat_id, text=self.get_text('enter_category'), reply_markup=reply_kb, parse_mode="HTML")
            except:
                m = await context.bot.send_message(chat_id=chat_id, text=self.get_text('err_invalid_number', val=input_value, ex_val=self.get_text('ex_stock_val')), reply_markup=cancel_kb, parse_mode="HTML")
            state['msg_id'] = m.message_id
            ...
            if step == 'waiting_simple_image':
                ...
                state['product_data']['image_url'], state['step'] = img if input_value != '-' else None, 'waiting_var_category'
                reply_kb = await self.get_existing_categories_keyboard()
                m = await context.bot.send_message(chat_id=chat_id, text=self.get_text('enter_category'), reply_markup=reply_kb, parse_mode="HTML")
                state['msg_id'] = m.message_id
    ...
    async def show_variant_type_selection(self, context, chat_id, user_id, status_msg="", edit_query=None):
        ...
        m = await context.bot.send_message(chat_id=chat_id, text=text, reply_markup=self.get_variant_type_keyboard(), parse_mode="HTML")
        state['msg_id'] = m.message_id
    ...
    async def handle_variant_type_selection(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        if data == "DONE":
            ...
            total_stock = sum(opt.get('qty', 0) if isinstance(opt, dict) else opt for v_type in vars_data for opt in vars_data[v_type].values())

            await self.conn.execute("INSERT INTO products (name, description, price, image_url, emoji, category, stock, variants) VALUES (?, ?, ?, ?, ?, ?, ?, ?)", (p.get("name"), p.get("description"), 0.0, p.get("image_url"), p.get("emoji", "📦"), p.get("category"), total_stock, json.dumps(vars_data, ensure_ascii=False)))
            await self.conn.commit()

            self.user_states.pop(user_id, None)
            await query.message.delete()
            return await context.bot.send_message(chat_id=query.message.chat_id, text=self.get_text('product_created_2', name=p.get('name')), reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton(self.get_text('back_button_3'), callback_data="admin_products")]]), parse_mode="HTML")
        ...
        await query.edit_message_text(text=self.get_text('variant_input_prompt', v_type=v_type_localized, example=ex), reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton(self.get_text('back_to_types_btn'), callback_data="admin_step_variants_init"), InlineKeyboardButton(self.get_text('cancel_button'), callback_data="admin_wizard_cancel")]]), parse_mode="HTML")
    ...
    async def admin_handle_variant_decision(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        msg = await context.bot.send_message(chat_id=query.message.chat_id, text=text, reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton(self.get_text('cancel_button'), callback_data="admin_wizard_cancel")]]), parse_mode="HTML")
        self.user_states[user_id]['msg_id'] = msg.message_id
    ...
    async def admin_back_to_variant_types(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        try: await query.edit_message_text(self.get_text('admin_wizard_variant_title', added_info=added_info), reply_markup=self.get_variant_type_keyboard(), parse_mode="HTML")
    ...
    async def get_existing_categories_keyboard(self, product_id=None):
        ...
        keyboard = [[InlineKeyboardButton(cat, callback_data=f"admin_set_cat_{cat}") for cat in categories[i:i + 2]] for i in range(0, len(categories), 2)]
        keyboard.append([InlineKeyboardButton(self.get_text('cancel_button'), callback_data=f"admin_prod_{product_id}" if product_id else "admin_wizard_cancel")])
        return InlineKeyboardMarkup(keyboard)

    async def admin_handle_category_selection(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        query = update.callback_query
        user_id = update.effective_user.id
        if int(user_id) not in ADMIN_IDS or user_id not in self.user_states: return await query.answer(self.get_text('session_expired'))

        category, state, chat_id = query.data.replace("admin_set_cat_", ""), self.user_states[user_id], query.message.chat_id
        await query.answer(self.get_text('admin_category_selected', category=category))

        if state.get('field') == 'category':
            await self.conn.execute("UPDATE products SET category = ? WHERE id = ?", (category, state.get('product_id')))
            await self.conn.commit()
            self.user_states.pop(user_id, None)
            ...
            if text == f"_admin_category_updated_success_": text = self.get_text('admin_category_updated', category=category)
            return await context.bot.send_message(chat_id=chat_id, text=text, reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton(self.get_text('back_button_3'), callback_data=f"admin_prod_{state.get('product_id')}")]]), parse_mode="HTML")
        ...
        if state.get('step') in ['waiting_simple_category', 'waiting_var_category']:
            ...
            state['step'] = 'waiting_simple_emoji' if state.get('step') == 'waiting_simple_category' else 'waiting_var_emoji'
            m = await context.bot.send_message(chat_id=chat_id, text=self.get_text('enter_emoji'), reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton(self.get_text('cancel_button'), callback_data="admin_wizard_cancel")]]), parse_mode="HTML")
            state['msg_id'] = m.message_id
    ...
    async def process_variant_values_input(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        try:
            ...
            for pair in text.split(','):
                p = pair.strip().split('=')
                if len(p) == 3: opts_map[p[0].strip()] = {"qty": int(p[1]), "price": float(p[2])}
                elif len(p) == 2: opts_map[p[0].strip()] = int(p[1])
                else: raise ValueError()
            ...
            ex = {"Size": "S=10=1200", "Color": "Red=5=500", "Memory": "128GB=10=800"}.get(v_type, self.get_text('variant_default_format'))
            ...
            m = await context.bot.send_message(chat_id=chat_id, text=text_err, reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton(self.get_text('back_button'), callback_data="admin_step_variants_init")]]), parse_mode="HTML")
            state['msg_id'] = m.message_id
    ...
    def main(self):
        ...
        application.add_handler(MessageHandler((filters.TEXT | filters.PHOTO) & ~filters.COMMAND, self.master_message_handler))
